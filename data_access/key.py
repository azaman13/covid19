...
def retrieve_question_data(column_name):
    answers = {}
    input_file_name = "data_access/survey_data_dict.csv"
    ...
    for raw_answer in answers_raw.split(" | "):
        ...
        answers[str(split_answer[0])] = split_answer[1]
    return question,answers


def retrieve_all_questions():
    question_key = {}
    input_file_name = "data_access/data_dict.csv"
    df = pd.read_csv(input_file_name)
...
def retrive_custom_user_groups(group_name,group_params):
    ...
    for i in group_params['categories']:
        user_responses[i] = {}
    userCount = 0
    files = glob.glob('data_access/user_search_data/*.csv')
    input_file_name = "data_access/processed_survey_data.csv"
    df = pd.read_csv(input_file_name)
...
def retrieve_user_groups(column_name):
    ...
    for i,row in df.iterrows():
        file = [file for file in files if str(row["record_id"])+"_" in file]
        df_temp = pd.read_csv(file[0])
        user_responses[str(row[column_name])][str(row["record_id"])] = df_temp
        userCount += 1 
    print("User Count:"+str(userCount))
    print("Question:",question)
    print("Key:")
    for i in answers.keys():
        print(str(i),"=",str(answers[i])+" | Percentage of Sample: "+str("{0:.0%}".format(len(user_responses[i].keys())/userCount)))
    return user_responses
